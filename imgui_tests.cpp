...
// FIXME-UTF8: Once ImGui supports codepoints above 0xFFFF we should only use 32bit codepoint testing variant.
bool CheckUtf8_cp16(const char* utf8, const char16_t* unicode)
{
    static_assert(sizeof(ImWchar) == sizeof(char16_t), "Tests expect ImWchar to be 2 bytes.");
    int utf8_len = strlen(utf8);
    int max_chars = utf8_len * 4 + 1;
    int result_bytes = 0;
    bool success = true;
    ...
    result_bytes = ImTextStrFromUtf8(converted, max_chars, utf8, NULL);
    success &= ImStrlenW((ImWchar*)unicode) == result_bytes && memcmp(converted, unicode, result_bytes * sizeof(ImWchar)) == 0;
